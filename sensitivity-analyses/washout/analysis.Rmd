...
```{r}
gam_test <- function(treatment, control, i) {
  d <- read_rds(glue::glue("{root_dir}/fig_data/tbl_{treatment}_{control}_{i}.rds"))
  m_null <- gam(y ~ 1, method = "ML", data = d)
  m_varying <- gam(y ~ s(x), method = "ML", data = d)
  ...
  mutate(treatment = treatment,
         control = control, 
         i = i) %>%
  select(treatment, control, i, d_AIC, d_BIC, statistic, p.value)
...
bind_rows(
  ...
  scale_y_log10() + 
  labs(x = "Lead-In Duration", y = "GAM p-value") + 
  theme_bw()

ggsave("~/projects/pd-preprint/figure_SR-gam.png", width = 8, height = 6)
```