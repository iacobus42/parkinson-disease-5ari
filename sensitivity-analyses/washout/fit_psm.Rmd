...
PS model fitting process. Compared to the "base" version of this script, I 
have added the argument `min_duration` which sets a minimum duration of follow
up *before* censoring or event to be able to enter the data set. 

```{r def-fit_gam_model}
fit_gam_model <- function(data, ps_formula, min_duration, k = 28) {
  data <- data %>%
    filter(survival_time >= min_duration)
...
Then generate all the data needed for matching with `min_duration` values of 1,
2, and 3 years.

```{r}
for (i in 1:3) {
  tz_tam_full <- fit_gam_model(tz_tam, ps_full, i * 365)
  tz_5ari_full <- fit_gam_model(tz_5ari, ps_full, i * 365)
  tam_5ari_full <- fit_gam_model(tam_5ari, ps_full, i * 365)

  write_rds(
    tz_tam_full,
    glue::glue("/Shared/lss_jsimmeri_backup/data/tz-5ari-final/matching_candidates/tz_tam_{i}.rds"))
  write_rds(
    tz_5ari_full,
    glue::glue("/Shared/lss_jsimmeri_backup/data/tz-5ari-final/matching_candidates/tz_5ari_{i}.rds"))
  write_rds(
    tam_5ari_full,
    glue::glue("/Shared/lss_jsimmeri_backup/data/tz-5ari-final/matching_candidates/tam_5ari_{i}.rds"))
}
```
