...
```{r def-ps-match}
ps_match <- function(treatment, control, seed, i) {
  log_path <- glue::glue("~/matching_logs/{treatment}_{control}_{i}.txt")
  ...
  candidates <- read_rds(glue::glue("/Shared/lss_jsimmeri_backup/data/tz-5ari-final/matching_candidates/{treatment}_{control}_{i}.rds"))
  ...
  write_rds(
    matches,
    glue::glue("/Shared/lss_jsimmeri_backup/data/tz-5ari-final/matches/{treatment}_{control}_{i}.rds")
  )
  cat(
...
par_match <- function(args) {
  ...
  seed <- args[[3]]
  i <- args[[4]]
  ps_match(treatment, control, seed, i)
  return(0)
}
```
...
```{r def-args-list}
set.seed(9894392)
seeds <- sample(1:10000000, 18)

args_list <- list(
  list("tz", "tam",   seeds[1],  0),
  list("tz", "5ari",  seeds[2],  0),
  list("tam", "5ari", seeds[3],  0),
  list("tz", "tam",   seeds[4],  1),
  list("tz", "5ari",  seeds[5],  1),
  list("tam", "5ari", seeds[6],  1),
  list("tz", "tam",   seeds[7],  2),
  list("tz", "5ari",  seeds[8],  2),
  list("tam", "5ari", seeds[9],  2),
  list("tz", "tam",   seeds[10], 3),
  list("tz", "5ari",  seeds[11], 3),
  list("tam", "5ari", seeds[12], 3),
  list("tz", "tam",   seeds[13], 4),
  list("tz", "5ari",  seeds[14], 4),
  list("tam", "5ari", seeds[15], 4),
  list("tz", "tam",   seeds[16], 5),
  list("tz", "5ari",  seeds[17], 5),
  list("tam", "5ari", seeds[18], 5)
)
```
