...
First, we need to compute the flags for the Charlson index:

```{r compute-charlson-flags}
charlson_flags_9 <- comorbid(
  lookback_dx9,
  icd9_map_charlson,
  visit_name = "enrolid",
  icd_name = "dx",
...
Which we then need to stack together:

```{r stack-charlson}
charlson_flags <- bind_rows(charlson_flags_9, charlson_flags_10) %>%
  gather(flag, value, -enrolid) %>%
  filter(value == TRUE) %>%
  distinct() %>%
  spread(flag, value) %>%
  mutate(across(everything(), function(x) replace_na(x, FALSE)))
```

And then use to calculate the weighted index - using Quan 2011 weights:

```{r compute-charlson-index}
cci_vector <- charlson_from_comorbid(
  charlson_flags,
  visit_name = "enrolid",
  scoring_system = "quan", 
  hierarchy = TRUE
...
cci <- tibble(
  enrolid = as.numeric(charlson_flags$enrolid),
  cci = cci_vector
)
```
...
Next, we want to add the CCI score. Not everyone has a defined CCI index (if
you had zero DX during lookback, you don't have a CCI index), so we use a
`left_join()` and a `replace_na()` to fix the implicit `FALSE` values.

```{r add-cci-index}
model_data <- left_join(model_data, cci, by = "enrolid") %>%
  mutate(cci = replace_na(cci, 0))
```
