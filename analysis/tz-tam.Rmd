...
survfit(Surv(survival_time, develops_pd) ~ drug, 
  ...
  select(year, Tamsulosin, `TZ/DZ/AZ`) %>%
  knitr::kable()
```
...
There is statistically significant correlation with the Schoenfeld residuals;
however, we are likely over-powered to detect a deviation. Looking at the 
estimated correlation (-0.059), we think it is unlikely that this has a 
meaningful impact on our estimated hazard ratio. 
